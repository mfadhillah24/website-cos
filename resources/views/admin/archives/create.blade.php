...
@section('title', 'Upload Arsip Baru')
@section('page_title', 'Upload Arsip Baru')
@section('breadcrumb', 'Arsip / Upload')
        ...
        <form action="{{ route('admin.archives.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 mb-4">
                <div>
                    ...
                    <input type="text" name="document_number" class="form-control @error('document_number') is-invalid @enderror" value="{{ old('document_number') }}" placeholder="Opsional">
                    @error('document_number')<div class="invalid-feedback">{{ $message }}</div>@enderror
                </div>
                <div>
                    <label class="form-label">Tanggal Dokumen</label>
                    <input type="date" name="document_date" class="form-control @error('document_date') is-invalid @enderror" value="{{ old('document_date') }}">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4 p-4 bg-gray-50 rounded-lg border border-gray-100">
                ...
                <div>
                    ...
                    <select name="activity_id" class="form-control @error('activity_id') is-invalid @enderror">
                        ...
                        @foreach($activities as $act)
                            <option value="{{ $act->id }}" {{ old('activity_id') == $act->id ? 'selected' : '' }}>{{ $act->title }}</option>
                        @endforeach
                    </select>
                ...
                <div>
                    ...
                    <select name="visibility" class="form-control @error('visibility') is-invalid @enderror" required>
                        <option value="internal" {{ old('visibility') == 'internal' ? 'selected' : '' }}>Internal (Semua Pengurus)</option>
                        <option value="restricted" {{ old('visibility') == 'restricted' ? 'selected' : '' }}>Restricted (Hanya Inti & Admin)</option>
                        <option value="public" {{ old('visibility') == 'public' ? 'selected' : '' }}>Public (Terbuka)</option>
                    </select>
            ...
            <div class="mb-6">
                ...
                <div class="form-text mt-1 text-xs text-gray-500">Format didukung: PDF, DOC/X, XLS/X, PPT/X, JPG, PNG. Maksimal 10MB.</div>
                @error('file')<div class="invalid-feedback">{{ $message }}</div>@enderror
            </div>

            <div class="flex justify-end gap-2">
                <a href="{{ route('admin.archives.index') }}" class="btn btn-secondary">Batal</a>
