...
@section('page_title', 'Edit RAB')
@section('breadcrumb', 'RAB / Edit')
...
<form method="POST" action="{{ route('admin.rabs.update', $rab) }}" id="rab-form">
@csrf @method('PUT')
...
<div class="grid-2" style="align-items:start;gap:24px;">
    ...
    <div class="card">
        <div class="card-header">
            <span class="card-title">Informasi RAB</span>
        ...
        <div class="card-body">
            ...
            <div class="form-group">
                ...
                <input type="text" name="activity_name" class="form-control @error('activity_name') is-invalid @enderror" value="{{ old('activity_name', $rab->activity_name) }}" required>
            ...
            <div class="form-group">
                ...
                <select name="period_id" class="form-control @error('period_id') is-invalid @enderror" required>
                    ...
                    <option value="{{ $period->id }}" {{ old('period_id', $rab->period_id) == $period->id ? 'selected' : '' }}>{{ $period->name }}</option>
                    @endforeach
                </select>
            ...
            <div class="form-group">
                ...
                <input type="date" name="date" class="form-control @error('date') is-invalid @enderror" value="{{ old('date', $rab->date?->format('Y-m-d')) }}" required>
                @error('date')<div class="invalid-feedback">{{ $message }}</div>@enderror
            </div>

            <div class="form-group">
                <label class="form-label">Penanggung Jawab <span style="color:#EF4444;">*</span></label>
                <input type="text" name="pic_name" class="form-control @error('pic_name') is-invalid @enderror" value="{{ old('pic_name', $rab->pic_name) }}" placeholder="Nama penanggung jawab" required>
            ...
            <div class="form-group">
                ...
                <input type="text" name="pic_position" class="form-control @error('pic_position') is-invalid @enderror" value="{{ old('pic_position', $rab->pic_position) }}" placeholder="Jabatan" required>
                @error('pic_position')<div class="invalid-feedback">{{ $message }}</div>@enderror
            </div>

            <div class="form-group" style="margin-bottom:0;">
                <label class="form-label">Keterangan</label>
                <textarea name="description" class="form-control" rows="3">{{ old('description', $rab->description) }}</textarea>
            </div>
...
<div style="display:flex;gap:12px;justify-content:flex-end;margin-top:8px;">
    <a href="{{ route('admin.rabs.show', $rab) }}" class="btn btn-secondary">Batal</a>
    <button type="submit" class="btn btn-primary">Perbarui RAB</button>
</div>
...
function addRow(desc='', qty='', unit='', price='') {
    ...
    tr.innerHTML = `
        ...
        <td style="padding:8px 12px;vertical-align:middle;border-bottom:1px solid var(--border-color);">
            <input type="text" name="items[${rowIndex}][description]" class="rab-input" style="${inputStyle}" placeholder="Uraian kegiatan..." value="${desc}" required>
        </td>
        ...
        <td style="padding:8px 12px;text-align:right;vertical-align:middle;border-bottom:1px solid var(--border-color);white-space:nowrap;">
            <span class="subtotal-display" style="font-size:13px;font-weight:600;color:var(--navy-900);">${formatRupiah(sub)}</span>
            <input type="hidden" name="items[${rowIndex}][subtotal]" class="subtotal-input" value="${sub}">
        </td>
...
// Populate existing items from database
@foreach($rab->items as $item)
addRow(
    @json($item->description),
    @json($item->quantity),
    @json($item->unit),
    @json($item->unit_price)
);
@endforeach

@if($rab->items->isEmpty())
addRow();
@endif
</script>
@endpush
