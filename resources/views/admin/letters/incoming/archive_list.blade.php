...
@section('page_title', 'Arsip Surat Masuk')
@section('breadcrumb', 'Administrasi / Surat Masuk / Arsip')
...
<div style="background: linear-gradient(135deg, #7c3aed, #5b21b6); border-radius:12px; padding:24px 28px; margin-bottom:24px; color:#fff; display:flex; align-items:center; gap:16px;">
    ...
    <div>
        ...
        <p style="font-size:13px;margin:0;opacity:0.85;">Lemari arsip digital — menampilkan seluruh surat masuk yang telah diarsipkan ({{ $letters->total() }} surat)</p>
    </div>
</div>
...
<div class="card">
    <div class="card-header">
        ...
        <a href="{{ route('admin.letters.incoming.index') }}" class="btn btn-secondary" style="display:flex;align-items:center;gap:6px;">
        ...
        <form method="GET" style="display:flex; gap:10px; flex-wrap:wrap; margin-bottom:20px;">
            ...
            <select name="letter_type" class="form-control" style="width:160px;">
                ...
                @foreach($letterTypes as $lt)
                    <option value="{{ $lt }}" @selected(request('letter_type') == $lt)>{{ $lt }}</option>
                @endforeach
            </select>
            @endif

            <button type="submit" class="btn btn-secondary">Filter</button>
            @if(request()->anyFilled(['search', 'year', 'letter_type']))
                <a href="{{ route('admin.letters.incoming.archive.index') }}" class="btn btn-secondary">Reset</a>
            @endif
        </form>
            ...
            <table>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td style="font-weight:500;">{{ $letter->letter_number }}</td>
                        <td>{{ $letter->sender }}</td>
                        ...
                        <td>
                            <div style="display:flex; gap:6px; flex-wrap:wrap;">
                                ...
                                {{-- Lampiran --}}
                                @if($letter->file_path)
                                    <a href="{{ route('file.serve', $letter->file_path) }}" target="_blank" class="btn btn-sm" style="background:#eff6ff;color:#1d4ed8;border:1px solid #93c5fd;">
                                        📎 Lampiran
                                    </a>
                                @endif

                                {{-- Kembalikan dari Arsip --}}
                                <form action="{{ route('admin.letters.incoming.unarchive', $letter) }}" method="POST" class="form-unarchive-archive">
                                    @csrf @method('PATCH')
                                ...
                                {{-- Hapus --}}
                                <form action="{{ route('admin.letters.incoming.destroy', $letter) }}" method="POST" class="form-delete-archive">
                                    @csrf @method('DELETE')
